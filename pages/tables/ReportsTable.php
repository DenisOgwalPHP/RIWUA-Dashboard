...
<head>
  ...
  <title>Crop Protection | Reports </title>
        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Reports Records</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               <?php
				require_once('../../Connection/connect.php');
				$sales6 ="SELECT Subject,Stories,Location,FullNames,Email,created_at,ImageURL,VideoURL,ID,Viewed FROM `story` order by ID Desc";
				...
				while($row5=mysqli_fetch_assoc($result5)){
				 ...
				 echo "<td><img src=".$row5['VideoURL']." height='100' width='100'></td>";
				 $Viewed=$row5['Viewed'];
				  $id=$row5['ID'];
					if($Viewed=='Viewed'){
						 echo "<td>".$row5['Viewed']."</td>";
					}else{				  
				  echo'<td><form method="get" ><a href="ReportsTable.php"><button type="submit" name="Edit"  value="'.$id.'" class="btn btn-block btn-warning">Viewed</button></a></form></td>'; 
					}
				 echo'<td><form method="get" ><a href="ReportsTable.php"><button type="submit" name="delete"  value="'.$id.'" class="btn btn-block btn-danger">Delete</button></a></form></td>'; 
                echo"</tr>";
				}
				?>
				<?php
				if(isset($_GET['delete'])){
					$id=$_GET['delete'];	
                        if(isset($id)){
				$sales6 ="DELETE  FROM story WHERE ID='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
				echo '<script type="application/javascript">';
				echo'alert("Report successfully Deleted");';
				echo'window.location.href="ReportsTable.php";';
				echo '</script>';
							}else{
				echo '<script type="application/javascript">';
				echo'alert("Report Delete Failed");';
				//echo'alert("'.$id.'");';
				echo'window.location.href="ReportsTable.php";';
				echo '</script>';
								}
				...
				$id=$_GET['Edit'];
				if(isset($id)){
				$sales6 ="UPDATE  story SET Viewed='Viewed' WHERE ID='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
					echo '<script type="application/javascript">';
				echo'alert("Report view Successful");';
				echo'window.location.href="ReportsTable.php";';
				echo '</script>';
				}else{
				echo '<script type="application/javascript">';
				echo'alert("Report view approval Failed");';
				echo'window.location.href="ReportsTable.php";';
				echo '</script>';
								}
				}
